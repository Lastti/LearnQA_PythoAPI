...
class TestUserAgentMethod:
    ...
    @pytest.mark.parametrize('user_agent, expected_values', testdata)
    def test_user_agent_method(self, user_agent, expected_values):
        print('user agent value: ', user_agent)
        ...
        assert 'browser' in response_dict, 'There is no "browser" field in the response'
        assert 'device' in response_dict, 'There is no "device" field in the response'
        ...
        assert actual_platform == expected_platform, f'Actual platform is not equal expected platform :{actual_platform}'
        assert actual_browser == expected_browser, f'Actual browser is not equal expected browser: {actual_browser}'
        assert actual_device == expected_device, f'Actual device is not equal expected device: {actual_device}'